...
from PyQt5.QtWidgets import (
    ...
    QFormLayout, QSpinBox  # Удалены виджеты для настроек DTX
)

# Локальные импорты
from voice_client_backend import VoiceClientBackend, pyaudio_available
from voice_client_constants import SERVER_ADDRESS, BITRATE  # Удалены константы DTX
...
class VoiceChatUI(QWidget):
    ...
    def setup_settings_menu(self):
        """Создает меню настроек БЕЗ DTX"""
        self.settings_menu = QWidget()
        self.settings_menu.setWindowTitle("Настройки голосового чата")
        # --- ИЗМЕНЕНО: Размер окна уменьшен, так как настройки DTX удалены ---
        self.settings_menu.setFixedSize(350, 250)  # Уменьшена высота
        # --- КОНЕЦ ИЗМЕНЕНИЯ ---
    ...
    def apply_settings(self):
        """Применяет настройки из меню настроек (БЕЗ DTX)"""
        # --- ИЗМЕНЕНО: Удалена логика применения настроек DTX ---
        # Получаем значения из виджетов
        # dtx_enabled = self.dtx_enabled_checkbox.isChecked()
        # dtx_threshold = self.dtx_threshold_spinbox.value()
        # dtx_max_quiet = self.dtx_max_quiet_spinbox.value()
        #
        # # Применяем настройки к текущему клиенту
        # if self.voice_client:
        #     self.voice_client.dtx_enabled = dtx_enabled
        #     self.voice_client.dtx_threshold = dtx_threshold
        #     self.voice_client.dtx_max_quiet_frames = dtx_max_quiet
        #     self.logger.info(f"Настройки DTX применены: включено={dtx_enabled}, порог={dtx_threshold}, макс.тихих={dtx_max_quiet}")
        # else:
        #     self.logger.warning("Клиент не подключен, настройки DTX не применены")
        #
        # # Здесь можно также сохранить настройки в файл или реестр для постоянного хранения
        # # Например, в voice_client_constants.py или отдельный config файл
        #
        # --- КОНЕЦ ИЗМЕНЕНИЯ ---
